...
describe("Browser tests", () => {
    ...
    it ("code flow with usePKCE = true", async () => {
        ...
        // pkce with clientSecret should fail
        expect(smart.authorize(env, {
            ...
            usePKCE: true
        })).to.reject("PKCE should only be used by public single-page web apps or native apps without a client secret");

        // Verify a valid pkce flow
        await smart.authorize(env, {
            iss: mockUrl,
            launch: "123",
            scope: "my_scope",
        ...
        expect(redirect.searchParams.get("code_challenge")).to.exist();
        expect(redirect.searchParams.get("code_challenge_method")).to.equal("S256");
        ...
        mockServer.mock({
            handler: (req, res) => {
                ...
                // We could return the response, but we want to throw errors to fail the test
                // if some query parameters are not expected.
                expect(req.body.code_verifier).to.exist();
            }
        });
